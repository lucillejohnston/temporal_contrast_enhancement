...
import eVAS_slider as evas # for VAS slider
    ...
    if os.path.exists(config_file):
        try:
            with open(config_file, 'r') as f:
                ...
                thermode_port = config.get('Thermode_port', None)
                vas_port = config.get('VAS_port', None)
...
class ExperimenterGUI:
    def __init__(self, patient_info, root, thermode, vas_slider, baseline, ramp_speed):
        self.root = root
        ...
        self.target_temps = [39, 25, 41, 23, 43, 21, 39, 23, 41, 25, 40, 18, 38, 16, 42, 12, 38, 20, 41] # I did this very randomly 
    ...
    def sample_loop(self):
        ...
        sensation = self.vas_slider.get_value()
...
if __name__ == '__main__':
    root = tk.Tk()
    experimenter_gui = ExperimenterGUI(patient_info, root, thermode,vas_slider, baselineTemp, ramp_speed)
    experimenter_gui.run()

temperatures = [39, 25, 41, 23, 43, 21, 39, 23, 41, 25, 40, 18, 42, 16, 44, 12, 38, 20, 41, 15, 40, 17, 42, 13, 39, 22, 41, 14, 43, 10]