...
# from TTL_pulse import open_serial_port, send_session_start_ttl, send_session_end_ttl, send_trial_ttl, send_event_ttl
    ...
    def submit(self):
        ...
        if config:
            ...
            if self.thermode_port is None:
                messagebox.showerror("Calibration Data Error","Thermode Port is missing in the config file")
                return
            if self.vas_port is None:
                messagebox.showerror("Calibration Data Error","VAS Port is missing in the config file")
                return
...
def get_patient_calibration():
    ...
    root.mainloop()
    return app.T1, app.T2, app.thermode_port, app.vas_port, app.num_blocks, app.side, app.trial_start, app.patient_info
...
def update_config(config_file, num_blocks):
    with open(config_file, 'r') as file:
        config = yaml.safe_load(file)
    if 'trial_order' in config:
        return config['trial_order']
...
class ExperimenterGUI:
    def __init__(self, root, thermode, vas_slider, T1, T2, patient_info, trial_start, trial_order_mapping, baseline, ramp_speed, return_speed): #, ttl_log):
        ...
        self.trials = list(trial_order_mapping.values())
    ...
    def start_trial(self):
        ...
        active_zones = [1,1,1,1,1] #next(target_generator)
    ...
    def run_trial_by_type(self, trial_type, active_zones):
        ...
        if not self.cancelled:
            ...
            if self.trial_number % 5 == 0: 
                self.log("Change thermode location")
    ...
    def run_trial_hold_T1(self, active_zones):
        ...
        # send_event_ttl(ttl_serial, self.ttl_log)
...
# # -------------------------------------
def main():
    T1, T2, thermode_port, vas_port, num_blocks, side, trial_start, patient_info = get_patient_calibration()
    if T1 is None:
        exit("Calibration data missing. Exiting.")
    # ttl_filename = f"{patient_info['name']}_{patient_info['date']}_ttl_log.csv"
    # ttl_log_path = os.path.join(os.path.dirname(__file__), "task_data", ttl_filename)
    ...
    vas_slider = evas.eVASSlider(vas_port)
    ...
    root = tk.Tk()
    experimenter_gui = ExperimenterGUI(root, thermode, vas_slider, T1, T2, patient_info,
