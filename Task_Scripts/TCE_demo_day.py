...
- Have participant slide the slider around to get used to it
- Just save the data
...
thermode_port = '/dev/tty.usbmodem11301'
#vas_port = 'COM6' ########## CHANGE TO MATCH DEVICE #########
vas_port = '/dev/tty.usbmodem11401'
"""
...
def make_config(patient_info, side):
    ...
    config = {
        ...
        "VAS_port": vas_port,
        "T1": 44, # just picked a temperature that shouldn't be too painful, hard coded for now not calibrated
...
def run_slider_practice(config_file):
    script_path = os.path.join(os.path.dirname(__file__), "slider_movement.py")
    if not os.path.exists(script_path):
        print(f"slider_movement.py not found at {script_path}")
        return
    try:
        ...
        print(f"Error running slider_movement.py: {e}")
...
if __name__ == '__main__':
    ...
    # PHASE 1
    run_slider_practice(config_file)
