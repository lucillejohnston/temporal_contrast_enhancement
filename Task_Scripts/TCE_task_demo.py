...
import yaml, time, threading, csv, os
...
class PatientInfoGUI:
    ...
    def submit(self):
        ...
        if config:
            ...
            self.T2 = self.T1 + 1
            self.thermode_port = config.get("Thermode_port",None)
            self.vas_port = config.get("VAS_port",None)
            self.trial_order = config.get("trial_order",{})
            self.trials = [self.trial_order[k] for k in sorted(self.trial_order.keys())]
            if self.T1 is None:
                messagebox.showerror("Config Data Error", "T1 value is missing in the config file")
                return  
            if self.thermode_port is None:
                messagebox.showerror("Config Data Error","Thermode Port is missing in the config file")
                return
            if self.vas_port is None:
                messagebox.showerror("Config Data Error","VAS Port is missing in the config file")
                return
            messagebox.showinfo("Config Loaded",f"T1 = {self.T1}; T2 = {self.T2}")
        else:
            messagebox.showerror("File Not Found",f"No config file found for {name} on {date_str}")
            return
        self.root.destroy()
...
def get_patient_config():
    ...
    root.mainloop()
    return app.T1, app.T2, app.thermode_port, app.vas_port, app.side, app.trial_start, app.patient_info, app.trials
    
# ---------------------------------
# Define Task Parameters
# ---------------------------------
baselineTemp = 25 # resting temperature in °C
ramp_speed = 1.5 # °C/s
return_speed = 6 # °C/s
# ---------------------------------
...
class ExperimenterGUI:
    def __init__(self, root, thermode, vas_slider, T1, T2, patient_info, side, trial_start, trials, baseline, ramp_speed, return_speed):
        self.thermode = thermode
        self.T1 = T1
        self.T2 = T2
        self.patient_info = patient_info
        self.vas_slider = vas_slider
        self.padding = 0.7 # seconds
        # Build the GUI
        self.root = root
    ...
    def run_trial_hold_T1(self, active_zones):
        # Ramp up from baseline to T1 and hold for 30 seconds
        ramp_duration = (self.T1 - self.baseline) / self.ramp_speed
        hold_duration = 30
        phaseABC_duration = ramp_duration + hold_duration
        self.thermode.set_durations([phaseABC_duration if active else 0 for active in active_zones])
        self.thermode.set_temperatures([self.T1 if active else self.baseline for active in active_zones])
        self.thermode.stimulate()
        self.log(f"Ramping up to ({self.T1}) over {ramp_duration:.2f}s")
    ...
    def run_trial_step_up(self, active_zones):
        ...
        hold_duration_B = 5
        hold_duration_C = 20
        total_duration = ramp_duration_A + hold_duration_A + ramp_duration_step + hold_duration_B + ramp_duration_step + hold_duration_C + self.padding

        self.thermode.enable_follow_mode()

        # Phase A: Ramp up from baseline to T1, hold for 5s
        self.thermode.set_durations([total_duration if active else 0 for active in active_zones])
        self.thermode.set_temperatures([self.T1 if active else self.baseline for active in active_zones])
        self.thermode.stimulate()
        ...
        # Phase B: Ramp up to T2, hold for 5s
        ...
        # Phase C: Ramp down to T1, hold for 20s
        ...
        ramp_down_duration = (self.T1 - self.baseline) / self.return_speed
        self.log(f"Ramping down to baseline ({self.baseline}) over {ramp_down_duration:.2f}s")
        self.thermode.disable_follow_mode()
        time.sleep(ramp_down_duration)
    ...
    def run_trial_step_down(self, active_zones):
        ...
        # Phase A: Ramp up from baseline to T2, hold for 5s
        self.thermode.set_durations([total_duration if active else 0 for active in active_zones])
        self.thermode.set_temperatures([self.T2 if active else self.baseline for active in active_zones])
        self.thermode.stimulate()
        ...
        # Phase B: Ramp down to T1, hold for 5s
        self.log(f"Ramping down to T1 ({self.T1}) over {ramp_duration_step:.2f}s")
        self.thermode.set_temperatures([self.T1 if active else self.baseline for active in active_zones])
        time.sleep(ramp_duration_step)
        ...
        # Phase C: Ramp up to T2, hold for 20s
        self.log(f"Ramping up to T2 ({self.T2}) over {ramp_duration_step:.2f}s")
        self.thermode.set_temperatures([self.T2 if active else self.baseline for active in active_zones])
        time.sleep(ramp_duration_step)
    ...
    def run_trial_hold_T2(self, active_zones):
        ...
        self.thermode.set_temperatures([self.T2 if active else self.baseline for active in active_zones])
        self.thermode.stimulate()
        self.log(f"Ramping up to ({self.T2}) over {ramp_duration:.2f}s")
        time.sleep(ramp_duration)
    ...
    def end_trial(self):
        ...
        data_dir = os.path.join(os.path.dirname(__file__), "..", "data","demo_day")
...
# -------------------------------------
def main():
    T1, T2, thermode_port, vas_port, side, trial_start, patient_info, trials = get_patient_config()
    if T1 is None:
    ...
    root = tk.Tk()
    experimenter_gui = ExperimenterGUI(root, thermode, vas_slider, T1, T2, patient_info, side, 
                                       trial_start=trial_start, trials=trials,
