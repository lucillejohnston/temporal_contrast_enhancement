...
def get_patient_calibration():
    ...
    root.mainloop()
    return app.T1, app.T2, app.com_port, app.side, app.trial_start, app.patient_info, app.trials
...
class ExperimenterGUI:
    def __init__(self, root, thermode, T1, T2, patient_info, side, trial_start, trials, baseline, ramp_speed, return_speed):
    ...
    def end_trial(self):
        ...
        data_dir = os.path.join(os.path.dirname(__file__), "demo_day_data")
...
# -------------------------------------
def main():
    T1, T2, com_port, side, trial_start, patient_info, trials = get_patient_calibration()
    if T1 is None:
    ...
    thermode.set_baseline(baselineTemp*5)
