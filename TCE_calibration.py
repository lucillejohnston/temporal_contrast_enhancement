...
#import VAS_slider as vas # midi slider
...
#thermode_port = '/dev/tty.usbmodem134101'
vas_port = 'COM6' ########## CHANGE TO MATCH DEVICE #########
#vas_port = '/dev/tty.usbmodem134201'
...
class ExperimenterGUI:
    def __init__(self, root, thermode, vas_slider, patient_info, baseline, ramp_speed, return_speed, hold_time, side):
        ...
        self.target_temp = 39 # Start with 40 C
    ...
    def sample_loop(self):
        ...
        #pain = self.vas_slider.get_value(self.side)
    ...
    def run_trial(self):
        ...
        self.active_zones = next(self.target_gen)
        self.log("Active zone: " + str(self.active_zones))
        self.ramp_up_phase()
        
    def ramp_up_phase(self): # --- Ramp-Up Phase ---
        self.log(f"Ramp-up: Stimulating {max(self.ramp_durations):.2f}s ramp-up")
        # Send temp to thermode
        self.thermode.set_durations([self.ramp_durations[i] if active else 0 for i,active in enumerate(self.active_zones)])
        ...
        # Schedule hold phase once ramp-up duration is over (minus small offset)
        delay_ms = int(max(self.ramp_durations) * 1000)
        self.root.after(delay_ms, self.hold_phase)
        
    def hold_phase(self): # --- Hold Phase ---
        if self.cancelled:
            self.trial_in_progress = False
            return
        self.log(f"Hold: Holding at {self.target_temp} C for {self.hold_time:.2f}s")
        self.in_hold_phase = True 
        # Send temp to thermode
        self.thermode.set_durations([self.hold_time if active else 0 for active in self.active_zones])
        self.thermode.set_temperatures([self.target_temp if active else self.baseline for active in self.active_zones])
        self.thermode.stimulate()
        delay_ms = int((self.hold_time) * 1000)
        self.root.after(delay_ms, self.ramp_down_phase)
    
    def ramp_down_phase(self): # --- Ramp-Down Phase ---
        self.in_hold_phase = False
        if self.cancelled:
            self.trial_in_progress = False
            return
        self.log(f"Ramp-down: Stimulating {max(self.ramp_down_durations):.2f}s ramp-down")
        # Send temp to thermode
        self.thermode.set_durations([self.ramp_down_durations[i] if active else 0 for i,active in enumerate(self.active_zones)])
        self.thermode.set_temperatures([self.baseline] *5)
        self.thermode.stimulate()
        ramp_down_ms = int(max(self.ramp_down_durations) * 1000)
        extra_sampling_ms = 5000  # extra delay in milliseconds
        delay_ms = ramp_down_ms + extra_sampling_ms
    ...
    def compute_rating(self):
        ...
        if self.hold_phase_samples:
            ...
            self.log(f"Average pain rating over the last 5 seconds: {avg_rating:.2f}")
    ...
    def adjust_temperature(self, rating):
        ...
        """
        if rating < 40:
            self.log(f"Pain rating {rating}. Increasing temp to {self.target_temp+1}")
        elif rating >= 60:
            self.log(f"Pain rating {rating}. Decreasing temp to {self.target_temp-1}")
            self.target_temp -=2 # Decrease temp by 1C
        elif 40 <= rating <= 60:
            self.log(f"Pain rating {rating} within range (40-60).\n")
...
if __name__ == '__main__':
    ...
    experimenter_gui = ExperimenterGUI(root, thermode, vas_slider, patient_info,
                                       ...
                                       return_speed = return_speed, hold_time = 30, side=side)
